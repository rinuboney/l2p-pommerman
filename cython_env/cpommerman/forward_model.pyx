...
cdef class ForwardModel(object):
    ...
    cdef np.ndarray get_features(self, const byte[constants.BOARD_SIZE][constants.BOARD_SIZE] board, list agents, list bombs,
        ...
        cdef np.ndarray features_np = np.zeros((4, constants.BOARD_SIZE, constants.BOARD_SIZE, 14), dtype=np.uint8)
        ...
        for row in range(constants.BOARD_SIZE):
            for col in range(constants.BOARD_SIZE):
                for i in range(9):
                    features[:, row, col, i] = (board[row][col] == i)

        for bomb in bombs:
            features[:,
                max(0, bomb.position.row-bomb.blast_strength):(bomb.position.row+bomb.blast_strength+1),
                bomb.position.col,
                9
            ] = bomb.life / 10
            features[:,
                bomb.position.row,
                max(0, bomb.position.col-bomb.blast_strength):(bomb.position.col+bomb.blast_strength+1),
                9
            ] = bomb.life / 10

        for agent in agents:
            features[agent.agent_id, :, :, 10] = agent.ammo
            features[agent.agent_id, :, :, 11] = agent.blast_strength
            features[agent.agent_id, :, :, 12] = agent.can_kick
            if agent.is_alive:
                features[agent.agent_id, agent.position.row, agent.position.col, 13] = 1
    ...
    @cython.wraparound(False)   # Deactivate negative indexing.
    @cython.initializedcheck(False)   # Deactivate memoryview initialization check.
    cdef list get_legal_actions(self, list agents, byte[constants.BOARD_SIZE][constants.BOARD_SIZE] board, list bombs):
        cdef characters.Bomber agent
        cdef characters.Bomb bomb
        cdef list actions
        cdef list agent_actions
        cdef tuple blockers = (constants.Item.Rigid, constants.Item.Wood, constants.Item.Bomb, constants.Item.Flames)
        cdef unsafe_positions = np.zeros((constants.BOARD_SIZE, constants.BOARD_SIZE), dtype=np.uint8)

        for bomb in bombs:
            if bomb.life == 1:
                unsafe_positions[max(0, bomb.position.row-bomb.blast_strength):(bomb.position.row+bomb.blast_strength+1), bomb.position.col] = 1
                unsafe_positions[bomb.position.row, max(0, bomb.position.col-bomb.blast_strength):(bomb.position.col+bomb.blast_strength+1)] = 1
            else:
                unsafe_positions[bomb.position.row][bomb.position.col] = 1

        actions = []
        for agent in agents:
            if not agent.is_alive:
                actions.append([constants.Action.Stop])
            else:
                agent_actions = []
                if unsafe_positions[agent.position.row][agent.position.col] != 1:
                    agent_actions.append(constants.Action.Stop)

                if agent.position.row - 1 >= 0 \
                 and board[agent.position.row - 1][agent.position.col] not in blockers \
                 and unsafe_positions[agent.position.row - 1][agent.position.col] != 1:
                    agent_actions.append(constants.Action.Up)

                if agent.position.row + 1 < constants.BOARD_SIZE \
                 and board[agent.position.row + 1][agent.position.col] not in blockers \
                 and unsafe_positions[agent.position.row + 1][agent.position.col] != 1:
                    agent_actions.append(constants.Action.Down)

                if agent.position.col - 1 >= 0 \
                 and board[agent.position.row][agent.position.col - 1] not in blockers \
                 and unsafe_positions[agent.position.row][agent.position.col - 1] != 1:
                    agent_actions.append(constants.Action.Left)

                if agent.position.col + 1 < constants.BOARD_SIZE \
                 and board[agent.position.row][agent.position.col + 1] not in blockers \
                 and unsafe_positions[agent.position.row][agent.position.col + 1] != 1:
                    agent_actions.append(constants.Action.Right)

                if agent.ammo > 0:
                    agent_actions.append(constants.Action.LayBomb)

                if len(agent_actions) == 0:
                    agent_actions = [constants.Action.Stop]
